...
<body class="font-sans bg-gray-50">
    <!-- Hero Section -->
    <section class="hero">
        <div style="position: relative; z-index: 1;" class="max-w-4xl mx-auto px-4 py-16">
            ...
            <div class="mt-12 bg-white rounded-lg shadow-lg p-8 max-w-4xl mx-auto">
                <h2 class="text-3xl font-bold text-gray-900 mb-6">Welcome to <?php echo $name; ?>!</h2>
                
                <?php if (isset($_SESSION['username'])): ?>
                    <p class="text-lg text-gray-800 mb-6">Welcome back, <?php echo htmlspecialchars($_SESSION['username']); ?>!</p>
                <?php else: ?>
                    ...
                    <ul class="list-disc list-inside mb-6 text-gray-700 space-y-2">
                        ...
                        <li>Learn new phrases and expressions</li>
                    </ul>
                    <div class="flex flex-wrap gap-4">
                        <a href="login.php" class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow hover:bg-blue-700 transition">Log In</a>
                        <a href="register.php" class="px-6 py-3 bg-gray-800 text-white font-semibold rounded-lg shadow hover:bg-gray-700 transition">Register</a>
                    </div>
                <?php endif; ?>
                ...
                <div class="mb-8">
                    <h3 class="text-2xl font-semibold text-gray-900 mb-4">Explore Languages</h3>
                    ...
                    <p id="disclaimer" class="mt-2 text-sm text-red-500"></p>
                </div>
                
                <!-- Language Grid will be populated here -->
                <div id="languageGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        <a href="language.php?id=<?php echo $row['lang_id']; ?>" 
                           class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:shadow-md transition-shadow">
                            <h3 class="text-xl font-semibold text-gray-900 mb-2"><?php echo htmlspecialchars($row['lang_name']); ?></h3>
                            <?php if (!empty($row['lang_place'])): ?>
                                <p class="text-gray-600 mb-2"><?php echo htmlspecialchars($row['lang_place']); ?></p>
                            <?php endif; ?>
                            <?php if (!empty($row['lang_info'])): ?>
                                <p class="text-gray-700 text-sm"><?php echo htmlspecialchars(substr($row['lang_info'], 0, 100)) . '...'; ?></p>
                            <?php endif; ?>
                        </a>
                    <?php endwhile; ?>
                </div>
    ...
    document.getElementById('langSearch').addEventListener('input', function(e) {
        ...
        const languages = document.querySelectorAll('#languageGrid > a');
        
        languages.forEach(lang => {
            const text = lang.textContent.toLowerCase();
        ...
        const visibleLanguages = document.querySelectorAll('#languageGrid > a[style="display: block;"]');
        const disclaimer = document.getElementById('disclaimer');
        
        if (visibleLanguages.length === 0 && searchTerm.length > 0) {
            disclaimer.textContent = 'No languages found matching your search.';
        } else {
